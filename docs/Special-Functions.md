...
:::tip[Note]
Both entities `heating_active` and `tapwater_active` can only be used on non-buffered systems.
If you have a system with a spare-tank at home, they will most likely not work as expected.
:::
...
- If the control is set to `roomthermostat` and the first temperature value for `remotetemp` is not sent within a minute, the master thermostat will likely show an error message.
